...
class CodeGenVisitor(BaseVisitor):
    ...
    def visitClassDecl(self,ast,c):
        ...
        if not init:
        # if "<init>" not in mem:
            self.genDEFAULTINITMETHOD(ast, self.env, Frame("<init>", VoidType() ))
        ### Tới method bth
    ...
    def genCLINITMETHOD(self, consdecl, o, frame):
        #included default init and init
        self.emit.printout(self.emit.emitMETHOD("<clinit>", MType([ClassType(Id(self.className))], VoidType(),Static()), True,frame))
        frame.enterScope(True)
    ...
    def visitCallStmt(self, ast, o):
        ...
        if type(ctype.skind) is Static:
            self.emit.emitGETSTATIC(cname + "/" + ast.method.name ,ctype, frame)
            self.emit.printout(in_[0])
            self.emit.printout(self.emit.emitINVOKESTATIC(cname + "/" + ast.method.name, ctype, frame))
        else:
            self.emit.emitGETFIELD(cname + "/" + ast.method.name ,ctype, frame)
            self.emit.printout(in_[0])
            self.emit.emitINVOKEVIRTUAL(cname + "/" + ast.method.name, ctype, frame)
        # self.emit.printout(in_[0])
        # self.emit.printout(in_[0])
        # self.emit.printout(self.emit.emitINVOKESTATIC(cname + "/" + ast.method.name, ctype, frame))
...
class GetName(BaseVisitor):
    ...
    def visitAttributeDecl(self, ast, o):
        ...
        if type(ctx) is VarDecl:
            return Symbol(ctx.variable.name,MType([],ctx.varType,ast.kind),CName(o))
        return Symbol(ctx.constant.name, MType([],ctx.constType,ast.kind),CName(o))
    ...
    def visitMethodDecl(self, ast, o):
        return Symbol(ast.name.name,MType([map(lambda x: self.visit(x,o),ast.param)],ast.returnType,ast.kind),CName(o))
